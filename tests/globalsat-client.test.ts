...
test("authenticates, validates targets and reuses the OAuth token", async () => {
  ...
  assert.equal(first[0].position?.recordedAt.toISOString(), "2026-09-17T13:30:00.000Z");
  assert.equal(second[0].id, 2619);
  assert.equal(calls.length, 3);
  assert.equal(calls[0].url, "https://apis.rastreioglobalsat.com/oauth/access_token");
...
test("uses date or cursor for tracking reports and sanitizes rate limits", async () => {
  ...
  await assert.rejects(
    client.getTrackingData({ targetIds: [5847], fromId: BigInt(1), limit: 100 }),
    (error: unknown) =>
